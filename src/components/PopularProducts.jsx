                ...
                {popularProductsData.map((product) => (
                    <Link key={product.id} to={`product/${product.id}`} className="flex hover:no-underline">
                        ...
                        <div className="ml-4 flex-1">
                            ...
                            <span
                                className={`text-sm font-medium ${product.product_stock === 0 ? 'text-orang-500' : 'text-green-400 '}`}
                            >
                                {product.product_stock === 0 ? 'OUt of Stock' : `${product.product_stock} in stock`}
                            </span>
                        </div>
                        <div>
                            <span className={`text-xs font-gray-400 pl-2 `}>{product.product_price}</span>
                        </div>
                    </Link>
                ))}
            </div>
